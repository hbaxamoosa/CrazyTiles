...
class GameFragment3x3 : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        viewModel.eventGameFinish.observe(viewLifecycleOwner) { isFinished ->
            if (isFinished) {
                ...
                val action = GameFragment3x3Directions.actionGameFragment3x3ToScoreFragment(
                    currentScore,
                    winner!!
                )
        ...
        binding.imageViewHelp.setOnClickListener {
            navController.navigate(GameFragment3x3Directions.actionGameFragment3x3ToInfoFragment())
        }
    ...
    override fun onStart() {
        ...
        when (val difficultyLevel: String = sharedPrefs.getString("list_preference", "unknown")!!) {
            "Easy" -> {
                findNavController().navigate(R.id.action_gameFragment3x3_to_gameFragment2x2)
            }
            "Medium" -> {
                // Firebase Analytics
                val bundle = Bundle()
            ...
            "Hard" -> {
                findNavController().navigate(R.id.action_gameFragment3x3_to_gameFragment4x4)
            }
            else -> {
